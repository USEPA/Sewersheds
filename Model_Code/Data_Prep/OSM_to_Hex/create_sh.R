...
for(n in 1:length(st.fips)){
  file.create(paste0("/work/GRDVULN/sewershed/Data_Prep/OSM_to_Hex/sh/FP_",st.fips[n],".sh"))
  fileConn<-file(paste0("/work/GRDVULN/sewershed/Data_Prep/OSM_to_Hex/sh/FP_",st.fips[n],".sh"))
  ...
  writeLines(c("#!/bin/bash -l",
               ...
               paste0("#SBATCH -e /work/GRDVULN/sewershed/Data_Prep/OSM_to_Hex/messages/",st.fips[n],".err"),
               paste0("#SBATCH -o /work/GRDVULN/sewershed/Data_Prep/OSM_to_Hex/messages/",st.fips[n],".out"),
               "",
               "module load intel/24.2 R/4.4.2 gdal geos hdf5 netcdf proj udunits",
               
               paste0("VAR='",st.fips[n],"'"),
               "export VAR",
               "Rscript /work/GRDVULN/sewershed/Data_Prep/OSM_to_Hex/OSM_to_Hex.R"), fileConn)
