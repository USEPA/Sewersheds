...
This document provides detailed descriptions of the full modeling process from development to the creation of the final dataset and is broken up into the following parts:

- **Overview of Existing Data**
  - Details the sewershed data that was obtained from public sources.
- **Selection of Validation Data**
  - Describes how existing data was reviewed and selected (or not selected) for inclusion in model development.
- **Feature Engineering**
  - Feature engineering refers to the methods used in obtaining and developing data that is relevant for accurately predicting whether an areas is served by a particular endpoint.
- **Model Development**
  - How the model was tuned, trained and validated to ensure the best possible predictions.
- **Constructing Sewersheds**
  - The output of the model is made up of millions of probabilities that must be evaluated and used to construct sewersheds from hexagons.
- **Results**
  - Detailed analysis of model performance and validation against existing sewershed data.
- **Discussion**
  - Implications of model results, discussion of modeling successes and limitations.

## Definitions

# Insert latex table of definitions with two columns
| Term | Definition |
|------|------------|
...
EPA conducted a rigorous search for publicly available sewershed data through online searches and engagement with stakeholders including federal and state agencies, regional partners, NGOs and academics working in the wastewater field. In total, we obtained 2,708 sewersheds from public sources which could be linked to the CWNS (@fig-stateSwr).

```{r stateSewersheds}
#| label: fig-stateSwr
#| fig-cap: "The total number of publicly available sewershed areas obtained by EPA for inclusion in the final dataset and for consideration in model training and testing."
#| fig-align: "center"
#| fig-alt: "A bar plot illustrating the count of sewersheds obtained from public sources. New York has the most with 600 of the total 2,708."
#| fig-width: 6
#| fig-height: 4
#| fig-show: "hold"

# state.swr <- st_read(here("Data/Final_CWNS_Utilities.gpkg"),
#                      layer = "State_Swr_04152025", quiet = TRUE)

# Load State Names
fips <- vroom(here("Data/fips_join.csv"), col_types = c("state_code"="c"))

state.swr.df <- vroom(here("Data/Final_CWNS_Utilities_04152025.csv"), show_col_types = FALSE)

state.table <- as.data.frame(table(substr(state.swr.df$CWNS_ID,1,2)))%>%
  left_join(fips, by = c("Var1"="state_code"))%>%
  mutate(state_name = fct_reorder(state_name,.x = state_name, .fun = sort))

ggplot(state.table)+
  geom_bar(aes(x = Freq, y = fct_rev(state_name)), stat = "identity", fill = "#4d8055")+
  labs(title = "Count of Publicly Available Sewersheds by State", x = "# of Systems", y = "")
...
ep <- st_read(here("Data/endpoints_04042025.shp"))%>%
  filter(STATE_CODE %in% intrsct.df$STATE_CODE)
