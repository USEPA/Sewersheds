...
As part of this survey, state-level teams submit technical information about publicly owned treatment plants. Some treatment plants may discharge a portion of their wastewater downstream to other treatment plants. When a treatment plant discharges less than 50% of its wastewater to another POTW, we refer to it as an 'endpoint', meaning that it is considered the final destination for wastewater prior to treatment and then discharge back into the environment, most frequently via direct discharge into a water body such as a river or ocean. The entire sewered area that contributes wastewater to a specific endpoint is referred to as a 'sewershed'. A sewershed may therefore include a single treatment plant and collection system or multiple treatment plants and their collection systems. The relationship between treatment plants and endpoints can be derived from CWNS reporting by accounting for reported discharge of wastewater between facilities.
...
EPA conducted a rigorous search for publicly available sewershed data through online searches and engagement with stakeholders including federal and state agencies, regional partners, NGOs and academics working in the wastewater field. In total, we obtained 3,280 sewersheds from public sources which could be linked to POTWs (@tbl-stateSwr).
...
ut.st.tbl <- utility.st %>%
  gt() %>%
  fmt_number(columns = c(Pop,nSystems),use_seps = TRUE,
             decimals = 0)%>%
  cols_label(state_name = "State",
             Pop = "Sewered Population of sourced Sewersheds",
             nSystems = "Count of Sourced Sewersheds",
             Pct = "% of State Sewersheds",
             Pop_Pct = "% of Sewered Population Served by Sourced Sewersheds")%>%
...
```
![sourced Sewersheds by State](img/Utility_Sourced_Sewersheds_by_State.png){#fig-utilitySewersheds fig-alt="A table showing the count of sourced sewersheds by state." fig-align="left" width=50%}
...
## Matching sourced Boundaries to CWNS Locations
...
To minimize computation time and increase efficiency, sourced sewersheds were used to test the maximum distance between an endpoint and the furthest possible hexagon that is served by that endpoint. 
...
## Validation
Two validations were performed to evaluate the percent of agreement by area and the percent of the sewered population captured by the estimated sewersheds. Sewersheds were modeled for the endpoints within the testing dataset and compared with their sourced versions. To calculate area, a spatial intersection was performed and percent capture was calculated as the percent of the area of the modeled sewershed that was within the sourced sewershed, divided by the total area of the sourced sewershed. The percent of the sewered population captured was calculated as the cumulative sum of the population within hexagons assigned to an endpoint divided by the total residential population as reported in the CWNS.
...
#| label: tbl-valid_pop
#| tbl-cap: "Percent of sewered population captured by modeled sewersheds compared with sourced sewersheds."
#| tbl-align: "center"
#| tbl-alt: "A table showing the percent of sewered population captured by modeled sewersheds compared with sourced sewersheds."
#| tbl-width: 6
#| tbl-height: 4
#| tbl-show: "hold"
...
utility.pop.counts %>%
  ...
  fmt_number(columns = c(Pop_T,Pop_U),use_seps = TRUE,
             decimals = 0)%>%
  cols_label(Pct = "Percent of Total Population Served by sourced Sewersheds",
             Pop_T = "Total Sewered Population",
             Pop_U = "Sewered Population of sourced Sewersheds")%>%
  gt_plt_bar(column = Pct, keep_column = FALSE, color = "#34a6ba")
```


### Sources of sourced Sewersheds

```{r stateSewersheds, eval = FALSE}
#| label: tbl-utilitySources
#| tbl-cap: "Sources for sourced Sewersheds."
#| tbl-alt: "A list of Sources for sourced Sewersheds."
...
We create a flat file where each row represents a POTW and perform the followng steps:
